        ...
        <div class="container">
            <div class="overview" v-show="pageShow">
                ...
                <div class="content-center">
                    <h3><router-link :to="{path: '/news/detail', query: { id: detailInfo.id, from: 'knowledge' }}">{{detailInfo.title}}</router-link></h3>
                    <span v-html="detailInfo.review"></span>
                </div>
            </div>
            <ul>
                <li class="news-panel-item" v-for="(item, index) in list" :data-index="index + 1" :key="index">
                    <router-link :to="{path: '/news/detail', query: { id: item.id, from: 'knowledge' }}" v-animate-piano="item.title"></router-link>
