        ...
        <div class="container" v-loading="loading" :loading-text="loadingText">
            <div class="overview" v-show="pageShow">
                ...
                <div class="content-center">
                    <h3><router-link :to="{path: '/news/detail', query: { id: detailInfo.id, from: 'evaluation' }}">{{detailInfo.title}}</router-link></h3>
                    <span>{{detailInfo.content}}</span>
                </div>
            </div>
            <ul>
                <li class="news-panel-item" v-for="(item, index) in list" :data-index="index + 1" :key="index">
                    <router-link :to="{path: '/news/detail', query: { id: item.id, from: 'evaluation' }}" v-animate-piano="item.title"></router-link>
